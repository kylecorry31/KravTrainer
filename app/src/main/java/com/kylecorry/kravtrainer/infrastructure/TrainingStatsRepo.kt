...
class TrainingStatsRepo(ctx: Context) {

    private val db = StatsDBHelper(ctx).writableDatabase

    fun getAll(): List<TrainingStats> {
        val cursor = query()
    ...
    fun create(stats: TrainingStats){
        db.insert(Constants.STATS_TABLE, null, getContentValues(stats))
    }

    fun update(stats: TrainingStats){
        val values = getContentValues(stats)
        db.update(Constants.STATS_TABLE, values, "${Constants.STATS_ID} = ?", arrayOf(stats.id.toString()))
    }
    ...
    private fun query(where: String? = null, whereArgs: Array<String>? = null, orderBy: String? = null): StatsCursor {
        val cursor = db.query(
            Constants.STATS_TABLE,
            null,
            where,
    ...
    private class StatsDBHelper(ctx: Context): SQLiteOpenHelper(ctx, Constants.DB_NAME, null, 1) {
